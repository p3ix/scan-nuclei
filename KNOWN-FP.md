...
| `templates/misconfiguration/tomcat/tomcat-manager-localhost-restriction-signal.yaml` | Si coincide junto a `tomcat-manager-html-exposed`, `tomcat-manager-text-endpoints-exposed` o `tomcat-manager-text-deploy-surface-exposed`, tratarlo como contexto de la misma superficie admin. Si solo aparece con `403`, informar como señal de despliegue existente pero con menor prioridad. |
| `templates/misconfiguration/tomcat/tomcat-host-manager-localhost-restriction-signal.yaml` | Igual que en `manager`, si la app ya esta confirmada por `200/401`, no contarlo aparte. Si solo hay `403`, usarlo como evidencia de superficie presente pero restringida. |
...
| `templates/exposures/sensitive-paths/tomcat-app-meta-inf-context-exposed.yaml` | Suele pertenecer a la misma familia que `tomcat-context-xml-exposed`, `tomcat-meta-inf-context-backups-exposed` o `tomcat-jndi-resources-exposed`; agrupar por aplicacion o descriptor y separar solo si cambia claramente la sensibilidad de la evidencia. |
...
| `templates/exposures/sensitive-paths/tomcat-config-archive-backups-exposed.yaml` | Si coincide junto a `tomcat-config-backups-exposed`, `tomcat-server-xml-exposed`, `tomcat-users-xml-exposed` o `tomcat-web-xml-exposed`, tratarlo como el mismo problema de backup/config expuesta, destacando aparte solo si el archivo comprimido es la evidencia principal o mas completa. |
| `templates/exposures/sensitive-paths/tomcat-meta-inf-context-backups-exposed.yaml` | Suele reforzar la misma fuga que `tomcat-context-xml-exposed` sobre `META-INF/context.xml`; deduplicar por aplicacion o descriptor salvo que el backup revele credenciales o recursos mas sensibles que el fichero activo. |
...
| `templates/exposures/sensitive-paths/tomcat-app-direct-archive-artifacts-exposed.yaml` | Igual que en `tomcat-app-archive-temp-artifacts-exposed`, pero fuera de `webapps/`. Puede describir la misma fuga de artefactos por aplicacion; agrupar por nombre de app y destacar por separado solo el archivo mas sensible o mas completo. |
| `templates/exposures/sensitive-paths/tomcat-default-text-artifacts-exposed.yaml` | `RELEASE-NOTES.txt`, `RUNNING.txt`, `BUILDING.txt`, `NOTICE.txt` o `LICENSE.txt` suelen reforzar la misma causa raiz que `tomcat-default-root-page-exposed`, `tomcat-examples-docs-exposed` o `tomcat-default-artifacts-exposed`; agruparlos como exposicion de contenido por defecto. |
| `templates/exposures/sensitive-paths/tomcat-runtime-logs-exposed.yaml` | Puede solaparse con `java-tomcat-stacktrace-exposed` o con otros logs/errores verbosos de la misma aplicacion. Priorizarlo mas cuando revele rutas, credenciales, nombres de app o stack traces persistentes, no solo el banner de arranque. |
...
| `templates/exposures/sensitive-paths/tomcat-app-web-inf-web-xml-exposed.yaml` | Variantes `/app/WEB-INF/web.xml`, `/api/WEB-INF/web.xml` y similares suelen describir la misma causa raiz que `web-inf-web-xml-exposed`, pero con mas valor por apuntar a aplicaciones reales. Agrupar por aplicacion y destacar por separado solo la evidencia mas sensible. |
| `templates/exposures/sensitive-paths/tomcat-app-directory-listing-exposed.yaml` | Si aparece junto a `tomcat-app-web-inf-web-xml-exposed`, `tomcat-app-meta-inf-context-exposed` o artefactos descargables de la misma app, tratarlo como evidencia de superficie/listing de la misma aplicacion, no como vulnerabilidad independiente adicional. |
